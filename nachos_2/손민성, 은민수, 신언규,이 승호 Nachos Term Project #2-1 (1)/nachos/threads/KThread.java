...
public class KThread {
	...
	public KThread() {
		if (currentThread != null) {
			...
			readyQueue = ThreadedKernel.scheduler.newThreadQueue(false); // 리디큐
																			// 생성
			readyQueue.acquire(this); // KThread에 cpu 할당해라->발산 ㄱㄱ

			currentThread = this; // currentThread를 KThread로 설정
			tcb = TCB.currentTCB(); //
			name = "main";
			restoreState();
	...
	private static class PingTest implements Runnable {
		...
		public void run() {
			for (int i = 0; i < 5; i++) {
				boolean intStatus = Machine.interrupt().disable();
				int priority = ThreadedKernel.scheduler.getPriority(currentThread);
				Machine.interrupt().restore(intStatus);
				System.out.println("*** 우선순위" + priority + "thread " + which + " looped " + i + " times");
				currentThread.yield();
			}
		}
		private int which;
...
*/
	...
	public static void selfTest(){
		...
		for(int i=0; i<10; i++){
			th[i] = new KThread(new PingTest(i));
			th[i].setName("priority" + i);
			th[i].fork();
		}	
	}
	*/
	...
	*/
	...
	public static void selfTest(){
		Lib.debug(dbgThread, "Enter KThread.selfTest");
		Lock threadOneLock = new Lock();
		Lock lock2 = new Lock();
		class donationTest implements Runnable{
			
			donationTest(int which){
				this.which=which;
			};
			donationTest(int which, Lock lock){
				this.which=which;
				this.lock = lock;
			}
			public void run() {
				...
				for (int i = 0; i < 5; i++) {
					...
					if(which==0 && i==2){
						...
						KThread priority3Thread = new KThread(new donationTest(1, lock2 ));
						Machine.interrupt().setStatus(false);                                                  
						ThreadedKernel.scheduler.setPriority(priority3Thread, 3);
						Machine.interrupt().setStatus(true);  
								
						priority3Thread.fork();
						
						
					}
					if(which == 1 && i==2){
						KThread priority7Thread = new KThread(new donationTest(2, threadOneLock));
						Machine.interrupt().setStatus(false);  
						ThreadedKernel.scheduler.setPriority(priority7Thread, 7);
						Machine.interrupt().setStatus(true);  
						priority7Thread.fork();
					}
		...
		KThread priority1Thread = new KThread(new donationTest(0, threadOneLock));
		Machine.interrupt().setStatus(false);
		ThreadedKernel.scheduler.setPriority(priority1Thread, 1);
		Machine.interrupt().setStatus(true);
		priority1Thread.fork();
